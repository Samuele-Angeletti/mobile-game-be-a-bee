...
public class BackgroundManager : MonoBehaviour
{
    ...
    {
        BackgroundContainer bgc = backgrounds.Find(bg => bg.sceneryType == sceneryType);
        if(bgc == null)
        {
            bgc = backgrounds[0];
            if (bgc == null)
    ...
    {
        if (newScenery == ActiveScenery) return; // aggiunto per la gestione del pulsante centrale
