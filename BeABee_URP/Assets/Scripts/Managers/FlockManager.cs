...
public class FlockManager : MonoBehaviour, ISubscriber, ISoundMaker
{
    ...
    private Bee SpawnBee(Vector3 position, bool isLeader, Sprite sprite)
    {
        ...
        newBee.Initialize(isLeader, sprite, this, bombAttackDestination.position, _currentInvulnerabilityTime > 0 ? _currentInvulnerabilityTime : bornInvulnerabilityTime);
    ...
    private void UpdateFlockSprite(ScenarioChoosedMessage scenarioChoosedMessage)
    {
        UpdateSprite(scenarioChoosedMessage.CurrentScenario);
        StartCoroutine(ChangeFlockSpriteCoroutine());
    }

    private IEnumerator ChangeFlockSpriteCoroutine()
    {
        for (int i = 0; i < _activeBeeList.Count; i++)
        {
            _activeBeeList[i].ChangeSprite(_currentSprite);
